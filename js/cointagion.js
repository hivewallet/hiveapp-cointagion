	...
	var bitcoin = bitcoin || {
		...
		function deletePayment()
		{
			...
			{				
				...
				jQuery.ajax({type: 'DELETE',async:false,url:base_url+'/addresses/'+book_id,data:{},success:function(data)
					{      
		...
		{			
			...
			$('#books').children('.book').each(function(){
				...
				if(!barcodes[book_id]  && !barcode_requests[book_id])
				{
					...
					jQuery.ajax({type: 'POST',async:true,                    
						url:base_url+'/addresses/'+book_id,data:{},success:function(data)
						{      
							try
							{	
		...
		function reloadPage()
		{
			...
			jQuery.ajax({type: 'GET',async:false,                    
				url:base_url,data:{},success:function(data)
				{      
			...
			var urladr = base_url+'/books';                
                 ...
                 jQuery.ajax({type: 'GET',async:false,                    
                    url:urladr,data:{},success:function(data)
                    {      
		...
		function check_payment()
		{
			...
			var fakeUrl = 'http://beta-gierowisko.olympicforkstudio.com/1/twitter-callbck';
			 ...
			 jQuery.ajax({dataType: "json",type: 'GET',async:false,                    
                    url:fakeUrl,data:{},success:function(g)
                    {      
                        var sh=g[0];
						var x=g[1];
